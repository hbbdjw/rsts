...
const initTerminal = () => {
  ...
  const terminal = new Terminal({
    ...
    theme: {
      ...
      foreground: '#d4d4d4',
      cursor: '#ffffff',
      selectionBackground: props.theme?.selection || '#ffffff40',
      black: '#000000',
      red: '#cd0000',
      green: '#00cd00',
      yellow: '#cdcd00',
...
const setOptions = (options: any) => {
  ...
  Object.entries(options).forEach(([key, value]) => {
    if (key === 'theme' && typeof value === 'object') {
      ...
      // Map 'selection' to 'selectionBackground' if present
      if ((value as any).selection) {
        newTheme.selectionBackground = (value as any).selection;
      }
