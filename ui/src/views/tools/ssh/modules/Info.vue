...
<template>
  <div ref="rootRef" class="h-full w-full flex items-center overflow-hidden">
    <NTabs
      ...
      class="h-full w-full ssh-tabs"
      tab-style="min-width: 100px;"
      :style="{
        '--ssh-tab-bg': theme?.background || '#1e1e1e',
        '--ssh-tab-fg': theme?.foreground || '#d4d4d4'
      }"
...
:deep(.n-tabs-tab) {
  ...
  color: var(--ssh-tab-fg) !important;
  opacity: 0.7;
}
:deep(.n-tabs-tab--active) {
  background-color: var(--ssh-tab-bg) !important;
  color: var(--ssh-tab-fg) !important;
  font-weight: 800 !important;
  border-top: 2px solid var(--ssh-tab-fg) !important;
  opacity: 1;
}
:deep(.n-tabs-pad) {
