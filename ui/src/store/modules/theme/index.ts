...
import { useDateFormat, useEventListener, useNow, usePreferredColorScheme, watchDebounced } from '@vueuse/core';
import { defineStore } from 'pinia';
import { defu } from 'defu';
import { getPaletteColorByNumber } from '@sa/color';
import { fetchUserThemeConfig, updateUserThemeConfig } from '@/service/api/user';
...
export const useThemeStore = defineStore(SetupStoreId.Theme, () => {
  ...
  /** Load theme config from backend */
  async function loadThemeConfig() {
    if (!authStore.isLogin) return;
    try {
      const { data, error } = await fetchUserThemeConfig();
      if (!error && data?.themeConfig) {
  ...
  /** Watch settings to save config */
  watchDebounced(
    settings,
    async () => {
      if (authStore.isLogin) {
        await updateUserThemeConfig(JSON.stringify(settings.value));
      }
